...
def my_c(message):
    if (check(message.chat.id, message.date)):
        if cv_user_id[message.chat.id][0] == 1:
            ...
            msg = bot.send_message(message.chat.id, "Выберите предмет:") #отправка преподу клавиатуры с его курсами
            bot.register_next_step_handler(msg, take_cours_pr)
def take_cours_pr(message): #препод выбрал предмет
    cv_user_id[message.chat.id][2]["course"] = message.text
    bot.send_message(message.chat.id, "Выберите действие")#, visual.start_keyboard) #Здесь отправляется преподавательская клава с вариантами действия функционала по курсу
...
@bot.message_handler(func=lambda message: message.text == "Редактировать курс")
def red(message):
    if (check(message.chat.id, message.date)):
